...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama) || empty($email) || empty($password) || empty($konfirm)) {
        ...
        if (mysqli_stmt_num_rows($chk) > 0) {
            ...
            mysqli_stmt_bind_param($ins, 'ssss', $nama, $email, $hash, $role);
...
<head>
    ...
    <title>Daftar Akun | KosKita</title>
    ...
    <div class="auth-content">
        <div class="auth-card-modern">
            ...
            <div class="mb-4 text-center">
                <span class="step-text text-center">KosKita Registration</span>
                <h1 style="margin-bottom: 0.5rem; text-align: center;">Sign up</h1>
            ...
            <form method="POST" action="">
                <!-- Role Tabs -->
                <div class="role-tabs">
                    <button type="button" class="role-tab-btn active" data-role="penyewa" onclick="setRole('penyewa')">Pencari Kos</button>
                    <button type="button" class="role-tab-btn" data-role="pemilik" onclick="setRole('pemilik')">Pemilik Kos</button>
                </div>
                <input type="hidden" name="role" id="roleInput" value="<?= htmlspecialchars($_POST['role'] ?? 'penyewa') ?>">
                <label class="form-label-modern" for="nama">Name*</label>
                <input type="text" id="nama" name="nama" class="form-control-modern" placeholder="Enter your full name" value="<?= htmlspecialchars($_POST['nama'] ?? '') ?>" required>
                ...
                <label class="form-label-modern" for="konfirmasi">Confirm Password*</label>
                <div class="input-icon-wrap">
                    <input type="password" id="konfirmasi" name="konfirmasi" class="form-control-modern" placeholder="Repeat your password" required>
                    <button type="button" class="toggle-pass" onclick="togglePwd('konfirmasi', 'iconPwd2')" tabindex="-1">
...
<script>
    ...
    function setRole(role) {
        ...
        document.querySelectorAll('.role-tab-btn').forEach(btn => btn.classList.toggle('active', btn.dataset.role === role));
    }
    setRole(document.getElementById('roleInput').value || 'penyewa');
</script>
</body>
</html>
